...
class AppStore extends Component {
  ...
  getFilteredApps = () => {
    ...
    const filteredapps = appsList.filter(each => each.category === ActiveTab)
    return filteredapps
  }
  ...
  render() {
    ...
    const searchResults = filteredapps.filter(eachitem =>
      eachitem.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    console.log(ActiveTab)
    return (
      <div className="background-contrainer">
